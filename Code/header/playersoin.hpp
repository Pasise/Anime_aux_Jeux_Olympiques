...
public:
    PlayerSoin(std::string lastname, std::string firstname, float Xp,float Xp_max,float Xp_multiplier, float speed, float x, float y, float damage, float soin, std::map<std::string, int> textureAssociations, std::string deathTexturePath);
    ~PlayerSoin() {};
    void doHeal();
